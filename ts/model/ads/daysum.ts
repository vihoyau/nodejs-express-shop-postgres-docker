...
export const defineFunction = function (sequelize: Sequelize) {
    return sequelize.define(modelName, {
        ...
        date: DataTypes.STRING, //日期，格式：YYYY-MM-DD
...
//插入一条记录
export async function insertDaySum(obj: any) {
    let re = await getModel(modelName).create(obj)
    return re ? re : undefined
}

//找这个广告商某天的汇总记录
export async function findByDateAndUUID(date: string, advertiseruuid: string) {
